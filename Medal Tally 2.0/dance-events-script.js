document.addEventListener("DOMContentLoaded", async () => {
    try {
        ...
        const danceEvents = data.filter(
            (event) => event.category && event.category.toLowerCase().includes("dances")
        );

        const danceSection = document.getElementById("dances-details");
        const tbody = danceSection.querySelector("tbody");
        tbody.innerHTML = "";

        if (danceEvents.length === 0) {
