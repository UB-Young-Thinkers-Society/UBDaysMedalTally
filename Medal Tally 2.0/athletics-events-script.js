document.addEventListener("DOMContentLoaded", async () => {
    try {
        ...
        const athleticEvents = data.filter(
            (event) => event.category && event.category.toLowerCase().includes("athletics")
        );

        const athleticSection = document.getElementById("athletics-details");
        const tbody = athleticSection.querySelector("tbody");
        tbody.innerHTML = "";

        if (athleticEvents.length === 0) {
