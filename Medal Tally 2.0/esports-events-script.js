document.addEventListener("DOMContentLoaded", async () => {
    try {
        ...
        const esportsEvents = data.filter(
            (event) => event.category && event.category.toLowerCase().includes("esports")
        );

        const esportsSection = document.getElementById("esports-details");
        const tbody = esportsSection.querySelector("tbody");
        tbody.innerHTML = "";

        if (esportsEvents.length === 0) {
            const tr = document.createElement("tr");
            tr.innerHTML = `<td colspan="5" style="text-align:center;">No esports events found</td>`;
      ...
      `;
        ...
        console.error("Error loading esports events:", err);
    }
});
